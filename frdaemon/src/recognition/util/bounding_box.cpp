...
std::vector<int> FaceInception::nms_max(std::vector<std::pair<Rect2d, float> > &rects, double overlap, FaceInception::IoU_TYPE type) {
...
Point2d FaceInception::project(Point3f pt, double focalLength, Point2d imgCenter) {
    cv::Point2d res;
...
Point3f FaceInception::transform(Point3f pt, Mat rot, Point3f trans) {
    cv::Point3f res;
...
bool FaceInception::calcCenterScaleAndUp(Mat faceData, std::vector<Point2d> imagePts, double normEyeDist, Point2d &center, double &scale, Point2d &upv) {
...
Rect2d FaceInception::calcRect(Mat faceData, std::vector<Point2d> imagePts) {
...
bool FaceInception::strict_weak_ordering(const std::pair<Rect2d, float> a, const std::pair<Rect2d, float> b) {
    return a.second < b.second;
}

void FaceInception::NMS(std::vector<std::pair<Rect2d, float> > &rects, double nms_threshold) {
...
std::vector<int> FaceInception::nms_avg(std::vector<std::pair<Rect2d, float> > &rects, double overlap) {
    const int n = rects.size();
    std::vector<double> areas(n);
...
double FaceInception::IoU(Rect2d rect, RotatedRect ellip, Size image_size) {
...
Mat FaceInception::getPyramidStitchingImage2(Mat &input_image, std::vector<std::pair<Rect, double> > &location_and_scale, double scaling, Scalar background_color, int min_side, int interval) {
    using namespace std;
...
void FaceInception::modernPosit(Mat &rot, Point3f &trans, std::vector<Point2d> imagePts, std::vector<Point3f> worldPts, double focalLength, Point2d center, int maxIterations) {
